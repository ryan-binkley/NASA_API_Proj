    ...
    return (
        ...
        {console.log(favVolcanos)}
            <div id='wrapperFavs'>
                ...
                <div id='favs'>
                    {favVolcanos.length ?
                        favVolcanos.map((volcano) => <li key={volcano.id}>{volcano.title} -----Added: {simpDate} <button onClick={() => setFavVolcanos( favVolcanos.filter((volcanos) => volcanos.id !== volcano.id))}>Remove</button></li>)
                        : defaultText}
                </div>
            </div>
        </>
